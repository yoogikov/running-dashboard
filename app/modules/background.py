...
This is the heatmap tab's content layer. Adapted for this repo's own root
module (a full-window frame everything else builds onto, unlike the
original's bare-root convention): surface=NONE, requires=("root",), and
the map widget is built onto root's frame instead of self.host.root
directly, so it stacks correctly under the (also root.frame-mounted) top
bar.
"""
import threading
...
class BackgroundModule(module.Module):
    ...
    def on_deps(self, deps):
        self.root_mod = deps["root"]

    def build(self, parent):
        self.map_widget = tkintermapview.TkinterMapView(
            self.root_mod.frame, width=1280, height=820, corner_radius=0,
